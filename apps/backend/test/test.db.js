...
async function checkDb() {
    ...
    for (const acc of accounts) {
        ...
        for (const r of reelsWithViews) {
            console.log(`    - Reel ${r.platformMediaId}: Views=${r.playCount}, Reach=${r.reach}, TotalWatchTime=${r.videoViewTotalTime}ms, AvgWatchTime=${r.avgWatchTime}ms, Likes=${r.likeCount}, Comments=${r.commentCount}`);
        }
    }
}
