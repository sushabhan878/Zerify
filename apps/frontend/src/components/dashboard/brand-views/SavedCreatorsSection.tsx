...
export default function SavedCreatorsSection() {
  ...
  const fetchSavedCreators = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/brand/saved-creators/detailed`, {
        headers: {
      ...
      if (res.ok) {
        ...
        if (Array.isArray(data)) {
          const formatted: CreatorItem[] = data.map((inf: any, idx: number) => {
          ...
          setSavedCreators(formatted);
          setIsLoading(false);
          return;
        }
      }
    } catch (err) {
      console.warn('Could not fetch saved creators from DB:', err);
    }
  ...
  const handleToggleBookmark = (creatorId: string) => {
    // Remove immediately from saved creators list
    setSavedCreators((prev) => prev.filter((c) => c.id !== creatorId));
  };

  // Filter & Sort Logic
  const filteredCreators = useMemo(() => {
    return savedCreators.filter((c) => {
      // 1. Search Query
      if (searchQuery.trim()) {
  ...
  }, [savedCreators, searchQuery, filters, sortBy]);
    ...
    <div className="space-y-4">
      ...
      {isLoading ? (
        <div className="min-h-[380px] flex items-center justify-center p-12">
          <LottieLoader size={200} message="Loading your saved creators library..." />
        </div>
        ...
        <div className="p-12 rounded-3xl bg-slate-900/60 border border-dashed border-white/10 text-center space-y-3">
          ...
          <h3 className="text-base font-black text-white">
            {savedCreators.length === 0 ? 'No saved creators in your library' : 'No saved creators match your filters'}
          </h3>
          <p className="text-xs text-slate-400 max-w-sm mx-auto">
            {savedCreators.length === 0
              ? 'Browse Find Influencers and click the heart icon on any creator card to save them to this roster library.'
              : 'Try adjusting your search query or clearing specific filter criteria.'}
          </p>
          {activeCount > 0 && (
            <button
